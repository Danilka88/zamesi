...
- **Реферальный блок монетизации в Studio** (`src/studio/referral.ts` + `src/studio/render/referral.ts`) — отдельная вкладка **«💰 Монетизация»** в панели «AI для редактора» (вкладки «📝 Публикация»/«💰 Монетизация»/«🚀 Продвижение», по умолчанию публикация). Панель расширена до **520 px** — читаемость секций выше. Товары из паспорта (`ecom_item`/`artist_merch`, до 6; fallback на `ad_targeting_keywords`) → **партнёрские ссылки на 3 магазина — Яндекс Маркет, AliExpress, Admitad** — в формате реальных API (ЯМ `/partner/link/create` с `clid`/`place_id`/`vid`, Ali `aliexpress.affiliate.link.generate` с `aff_trace_key`, Admitad `/deeplink/{w_id}/{c_id}/` с `subid`/`ulp`) + короткие ссылки (`ya.cc/...`, `s.click.aliexpress.com/...`, `ad.admitad.com/r/...`). Наглядный UI: 4 KPI (потенциал ₽ / ссылок / ср. CTR / ср. комиссия), donut-диаграмма распределения дохода по магазинам (`conic-gradient`), карточка каждого товара с **табами магазинов** (переключение без перерисовки), прогнозом (CTR/конверсия/охват/EPC/доход — детерминировано, NFR-7) и кнопками «Копировать»/«Открыть». Глобальные действия: «Копировать все ссылки», «Вставить блок в описание» (React-safe, `setNativeValue`), «Экспорт CSV». Детерминированный прогноз: комиссия по диапазону магазина, CTR 2.5–7%, конверсия 0.8–3.2%, охват из `confidence`, средний чек 12 000 ₽. Всё офлайн (NFR-7); реальные OAuth-ссылки подключатся к `/partner/link/create` и т.п. без переделки UI.
- **Продвижение видео в Studio** (`src/studio/promotion.ts` + `src/studio/render/promotion.ts`) — вкладка **«🚀 Продвижение»**: из паспорта (seo_title, scene_summary, ad_targeting_keywords, target_audience, brand_safety) собираются **готовые объявления для 3 рекламных площадок — Яндекс Директ, VK Реклама, MyTarget** — в форме их реальных API (`POST /json/v5/campaigns`, `ads.create`, `/api/v2/campaigns.json`), с лимитами заголовков (33/40/35 зн) и текста. Наглядный UI: 4 KPI (охват/клики/CTR/бюджет), donut распределения бюджета, карточка кампании с **превью видеокреатива** (опорный кадр из первой сцены + длительность), варианты объявлений (табы 1/2), **бюджет-слайдер** (500–10000 ₽) с блоком «Рекомендуем N ₽/день — …» из паспорта (домен/слоты/длительность) и live-пересчётом прогноза (CTR/CPC/CPM/охват/клики/CPA), таргетинг-чипы (возраст/интересы/гео), UTM-ссылка. Действия: «Копировать все», «⬇ CSV», «⬇ JSON» (в форме запроса к API), **«🚀 Отправить (демо)»** с toast про OAuth, «вставить UTM в описание». Всё офлайн и детерминировано (NFR-7).
- **Тренды и плейлисты в Studio** (`src/studio/trends.ts` + `src/studio/render/trends.ts`) — вкладка **«🔥 Тренды»**: идеи для новых видео из демо-Яндекс.Wordstat + Google Alerts (курируемый файкстур, NFR-7). UI: 4 KPI (растущих трендов / средний рост / коллабораций / плейлистов), donut распределения по сложности (Низкая/Средняя/Высокая), карточка тренда (объём показов, рост WoW ▲/▼, сложность, сезонность-спарклайн 12 месяцев, источники, связанные запросы, прогноз CTR), фильтры «Все/Растущие/Alerts». Секция **«📋 Плейлисты и подборки»** — автоплейлисты паспорта + тренд-подборка месяца «Велосипед» с реальными обложками (`public/bike/*.png`), 3 видео и «путём зрителя» (Новичок/Продвинутый/Профи с кликабельными тайм-кодами `?start=`). Секция **«🤝 Коллаборации»** — 6 синтетических каналов RUTUBE с пересечением аудиторий (overlap-бар), готовым питчем для переписки. Действия: «⧉ Копировать запрос», «+ В заголовок» (applyValue), «⧉ Структура плейлиста», «+ В поиск плейлиста», питч коллаборации. Всё детерминировано (один videoId → один результат); живой прокси Wordstat/Alerts подключается позже без переделки UI.
...
| `studio-referral.test.ts` | реферальный блок: 3 магазина + URL по форматам API, детерминизм, комиссии/прогноз, fallback-товары, CSV, блок описания |
| `studio-promotion.test.ts` | продвижение: 3 площадки + лимиты, прогноз (CTR/клики≤охват), UTM, рекомендация бюджета, бандл 3×≥2 креатива, CSV/JSON, детерминизм |
| `studio-render.test.ts` | панель AI: таббар «Публикация»/«Монетизация», рендер секции «Монетизация · Реферальные ссылки», кнопки действий, «вставить блок в описание», табы магазинов |

Проверка: `npm run typecheck && npm test` (222 теста, зелёные).
