...
export interface MonetizationItem {
  type: 'ad_slot' | 'ecom_item' | 'clip_candidate' | 'music_track' | 'artist_merch' | 'event_ticket' | 'celebrity_appearance'
...
export interface AudioData {
  music_matches: TrackMetadata[]
  celebrity: CelebrityVoice | null
}

export interface MixSceneRef {
  video_id: string
  scene_index: number
  start_sec: number
  end_sec: number
...
export interface FlatMonetizationItem {
  scene_index: number
  type: string
  search_query: string | null
  confidence: number | null
